...
\begin{document}
  ...
  much happier if you do. The instructions are posted on the course
  website and webcourses, and you should read them carefully. Here is a \href{https://www.dropbox.com/scl/fi/gpwv8s5b6xx16cepm1bn9/submission_instructions.pdf?rlkey=m3l066bopj9bu1l9m53rbcucs&raw=1}{direct link to the instructions}.
  ...
  \begin{answer}
    Most of the positve tests come from false positives.
  \end{answer}
  ...
  Asked claude sonnet 5 "how can I handle cases where there is one zero"
  \begin{lstlisting}
    def bar_grad(x):
      ...
      if numZeros == 0:
          ...
          grad[indZero] = np.prod(np.delete(x, indZero)) 
  ...
  \begin{answer} \\
    Highest mean  = WtdILI with $1.567$ \\
    Lowest mean  = Pac with $1.063$ \\
    Highest variance  = Mtn with $0.799$ \\
    Lowest variance = Pac with $0.322$ 
  ...
  \begin{answer}
    No, since with continous data, values that are exactly the same occur in a way that is basically random. A better measure would be to sort the data into ranges of values, and then observe which range has the most values fall in it.
  \end{answer}
  ...
  \begin{answer}
    D, sicne it's the only histogram showing \emph{each} column.
  \end{answer}
  ...
  \begin{answer}
    C, since it is the only box plot
  \end{answer}
  \item A plot showing the illness percentages over time.
  \begin{answer}
    A, since it is the only "plot"
  \end{answer}
  \item A scatterplot between the two regions with highest correlation.
  \begin{answer}
    F, since out of the two scatterplots, it has the higher correlation.
  \end{answer}
  \item A scatterplot between the two regions with lowest correlation.
  \begin{answer}
    E, since out of the two scatterplots, it has the lower correlation.
  \end{answer}
  \end{enumerate}
  ...
  \begin{answer}
    An equality based splitting rule makes sense to use with nominal features, like which party each state voted for in the last election cycle.
  \end{answer}
  ...
  \begin{answer}
    In terms of runtime, DecisionStumpErrorRate was the slowest, then DecisionStumpInfoGain was slightly faster, then scikit-learn's was the fastest by a massive margin. The error rates show that DecisionStumpErrorRate had the highest error, plateauing much quicker and higher than DecisionStumpInfoGain and scikit. In fact, the latter two have the exact same curve.
    Approach $(1)$ has disappointing results because the way it splits the data doesn't guaruntee that both children don't predict the same thing, while DecisionStumpInfoGain and scikit actively try to separate the classes.
  \end{answer}
  ...
  \begin{answer}
    No, having the same curve doesn't guaruntee the two are the same. 6.5 itself disproves this, with DecisionStumpInfoGain and scikit having the same curve, but being different, for example scikit was much, much faster. An experiment thatr could build confidence in two models being equivalent could be to check each model at each node and see how they are splitting the data. If they do the same for each node, we can be a lot more confident that the two are equivalent.   
  \end{answer}
\end{document}
