...
def q5_1():
    ...
    # YOUR CODE HERE
    flat = X.flatten() #To get for ALL values
    print("Min: %.3f" %np.min(flat))
    print("Max: %.3f" %np.max(flat))
    print("Mean: %.3f" %np.mean(flat))
    print("Median: %.3f" %np.median(flat))
    print("Mode: %.3f" %mode(flat))

    #Asked claude sonnet 5 "how can i get the quantiles of all values across a csv using numpy"
    quantiles = [5, 25, 50, 75, 95]
    q_values = np.percentile(flat, quantiles)
    for q, v in zip(quantiles, q_values):
        print(f"{q}% quantile: {v:.3f}")

    #Asked claude sonnet 5 "how can i get the region-level stats"
    means = df.mean(axis=0)
    variances = df.var(axis=0)
    print("Region with highest mean:", means.idxmax(), "(%.3f)" % means.max())
    print("Region with lowest mean: ", means.idxmin(), "(%.3f)" % means.min())
    print("Region with highest variance:", variances.idxmax(), "(%.3f)" % variances.max())
    print("Region with lowest variance: ", variances.idxmin(), "(%.3f)" % variances.min())
